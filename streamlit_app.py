...
with st.form("triage_form"):
    ...
    temp,hr = st.columns (2)
    with temp:
        temp_level = st.selectbox ("Temperature",options=list(temp_map.keys()),index=1)
    with hr:
        hr_level = st.selectbox("Heart Rate", options=list(hr_map.keys()),index=1)
...
if submitted:
    patient = pd.DataFrame([{
        ...
        'temperature_level': temp_map.get(temp_level, 1),
        'heart_rate_level' : hr_map.get(hr_level, 1),
